...
function(data) {
    ...
    console.log('get!') $('#choo').editableSelect();
    choo.value = "选择一个文件或直接新增一个文件"choo.disabled = false
});
$(function() {
    $("#mdeditor").markdown({
        language: 'zh'
...
function getdoc() {
    choo.disabled = true mdeditor.disabled = true url = "/hpp/admin/api/getdoc/" + choo.value + "?_" + Date.now(new Date()) loadMarkdown(url);
    choo.disabled = false mdeditor.disabled = false
};
function hpp_uploadmarkdown() {
    choo.disabled = true mdeditor.disabled = true
    var ajax = ajaxObject();
